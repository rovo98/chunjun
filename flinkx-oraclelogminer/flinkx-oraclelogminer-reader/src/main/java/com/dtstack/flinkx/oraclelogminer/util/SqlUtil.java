...
public class SqlUtil {
    ...
    /** 启动logminer 视图说明： v$log：存储未归档的日志 v$archived_log：存储已归档的日志文件 v$logfile： */
    public static final String SQL_START_LOG_MINER =
            ""
                    + "DECLARE\n"
                    + "    st          BOOLEAN := true;\n"
                    + "    start_scn   NUMBER := ?;\n"
    ...
    public static final String SQL_START_LOG_MINER_10 =
            ""
                    + "DECLARE\n"
                    + "    st          BOOLEAN := true;\n"
                    + "    start_scn   NUMBER := ?;\n"
    ...
    public static final String SQL_GET_LOG_FILE_START_POSITION_BY_SCN =
            "select min(FIRST_CHANGE#) FIRST_CHANGE# from (select FIRST_CHANGE# from v$log where ? between FIRST_CHANGE# and NEXT_CHANGE# union select FIRST_CHANGE# from v$archived_log where ? between FIRST_CHANGE# and NEXT_CHANGE# and standby_dest='NO'  and name is not null)";

    public static final String SQL_GET_LOG_FILE_START_POSITION_BY_SCN_10 =
            "select min(FIRST_CHANGE#) FIRST_CHANGE# from (select FIRST_CHANGE# from v$log where ? > FIRST_CHANGE# union select FIRST_CHANGE# from v$archived_log where ? between FIRST_CHANGE# and NEXT_CHANGE# and standby_dest='NO' and name is not null)";
    ...
    // 修改当前会话的date日期格式
    public static final String SQL_ALTER_DATE_FORMAT =
            "ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD HH24:MI:SS'";

    // 修改当前会话的timestamp日期格式
    public static final String NLS_TIMESTAMP_FORMAT =
    ...
    private static String buildSchemaTableFilter(String listenerTables) {
        ...
        for (String tableWithSchema : tableWithSchemas) {
            ...
            if (!ConstantValue.STAR_SYMBOL.equals(tables.get(1))) {
                tableFilterBuilder
                        .append(" and ")
                        .append(String.format("TABLE_NAME='%s'", tables.get(1)));
            }
