...
public class LocalTest {
    ...
    public static JobExecutionResult runJob(
            File jobFile, Properties confProperties, String savepointPath) throws Exception {
        String jobContent = readJob(jobFile);
        return runJob(jobContent, confProperties, savepointPath);
    }

    public static JobExecutionResult runJob(
            String job, Properties confProperties, String savepointPath) throws Exception {
        ...
        if (needRestart(config)) {
            env.setRestartStrategy(
                    RestartStrategies.fixedDelayRestart(10, Time.of(10, TimeUnit.SECONDS)));
        }
    ...
    private static BaseDataReader buildDataReader(
        ...
        switch (readerName) {
            ...
            case PluginNameConstants.GREENPLUM_READER:
                ...
                break;
            case PluginNameConstants.PHOENIX5_READER:
                reader = new Phoenix5Reader(config, env);
                break;
            case PluginNameConstants.KINGBASE_READER:
                reader = new KingbaseReader(config, env);
                break;
            case PluginNameConstants.ORACLE_LOG_MINER_READER:
                reader = new OraclelogminerReader(config, env);
                break;
            case PluginNameConstants.RESTAPI_READER:
                reader = new RestapiReader(config, env);
                break;
            case PluginNameConstants.SQLSERVER_CDC_READER:
                reader = new SqlservercdcReader(config, env);
                break;
            default:
    ...
    private static BaseDataWriter buildDataWriter(DataTransferConfig config) {
        ...
        switch (writerName) {
            ...
            case PluginNameConstants.GREENPLUM_WRITER:
                ...
                break;
            case PluginNameConstants.PHOENIX5_WRITER:
                writer = new Phoenix5Writer(config);
                break;
            case PluginNameConstants.KINGBASE_WRITER:
                writer = new KingbaseWriter(config);
                break;
            case PluginNameConstants.RESTAPI_WRITER:
                writer = new RedisWriter(config);
                break;
            case PluginNameConstants.ALLUXIO_WRITER:
                writer = new AlluxioWriter(config);
                break;
            case PluginNameConstants.OSS_WRITER:
                writer = new OssWriter(config);
                break;
            default:
