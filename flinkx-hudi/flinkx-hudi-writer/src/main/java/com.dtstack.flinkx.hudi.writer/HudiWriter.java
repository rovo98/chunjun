...
public class HudiWriter extends BaseDataWriter {
    /** org.apache.hudi.common.model.WriteOperationType UPSERT | UPSERT_PREPPED */
    ...
    public HudiWriter(DataTransferConfig config) {
        ...
        tableType =
                writerConfig
                        .getParameter()
                        .getStringVal(
                                HudiConfigKeys.KEY_TABLE_TYPE,
                                HoodieTableType.COPY_ON_WRITE.name());
        recordKey =
                writerConfig.getParameter().getStringVal(HudiConfigKeys.KEY_TABLE_RECORD_KEY, "id");
        path = writerConfig.getParameter().getStringVal(HudiConfigKeys.KEY_PATH);
        compress =
                writerConfig
                        .getParameter()
                        .getBooleanVal(HudiConfigKeys.KEY_COMPRESS, Boolean.FALSE);
        ...
        String partitionField =
                writerConfig.getParameter().getStringVal(HudiConfigKeys.KEY_PARTITION_FIELDS);
        partitionFields =
                StringUtils.isNotBlank(partitionField)
                        ? Arrays.asList(StringUtils.split(partitionField, ","))
                        : new ArrayList<>();
        hadoopConfig =
                (Map<String, Object>)
                        writerConfig.getParameter().getVal(HudiConfigKeys.KEY_HADOOP_CONFIG);
    }
