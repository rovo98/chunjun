...
package com.dtstack.flinkx.es7x.writer;
...
/**
 * The writer plugin of ElasticSearch 7.x
 *
 * <p>Company: www.dtstack.com
 *
 * @author huyifan.zju@163.com
 */
public class Es7xWriter extends BaseDataWriter {
    ...
    public Es7xWriter(DataTransferConfig config) {
        super(config);
        ...
        bulkAction =
                writerConfig
                        .getParameter()
                        .getIntVal(EsConfigKeys.KEY_BULK_ACTION, DEFAULT_BULK_ACTION);

        clientConfig = new HashMap<>();
        clientConfig.put(
                EsConfigKeys.KEY_TIMEOUT,
                writerConfig.getParameter().getVal(EsConfigKeys.KEY_TIMEOUT));
        clientConfig.put(
                EsConfigKeys.KEY_PATH_PREFIX,
                writerConfig.getParameter().getVal(EsConfigKeys.KEY_PATH_PREFIX));
        ...
        if (idColumns != null && idColumns.size() != 0) {
            ...
            for (int i = 0; i < idColumns.size(); ++i) {
                Map<String, Object> sm = (Map) idColumns.get(i);
    ...
    public DataStreamSink<?> writeData(DataStream<Row> dataSet) {
        Es7xOutputFormatBuilder builder = new Es7xOutputFormatBuilder();
