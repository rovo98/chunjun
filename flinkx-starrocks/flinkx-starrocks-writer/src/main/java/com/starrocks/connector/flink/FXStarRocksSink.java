...
import com.starrocks.connector.flink.table.sink.FXSinkFunctionFactory;
...
public class FXStarRocksSink {
     ...
     * @param sinkOptions StarRocksSinkOptions as the document listed, such as jdbc-url, load-url,
     *     batch size and maximum retries
     * @param rowDataTransformer StarRocksSinkRowBuilder which would be used to transform the
     *     upstream record.
    ...
    public static <T> SinkFunction<T> sink(
            ...
            // Add for FlinkX feature.
            List<String> preSql,
            List<String> postSql) {
        StarRocksIRowTransformer<T> rowTransformer =
                new StarRocksGenericRowTransformer<>(rowDataTransformer);
        return FXSinkFunctionFactory.createSinkFunction(
                sinkOptions, flinkTableSchema, rowTransformer, preSql, postSql);
    }
    ...
    public static SinkFunction<String> sink(StarRocksSinkOptions sinkOptions) {
        return FXSinkFunctionFactory.createSinkFunction(sinkOptions);
    }

    private FXStarRocksSink() {}
}
