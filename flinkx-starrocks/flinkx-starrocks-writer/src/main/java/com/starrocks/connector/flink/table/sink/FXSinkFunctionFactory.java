...
public class FXSinkFunctionFactory {

    private static final Logger LOG = LoggerFactory.getLogger(FXSinkFunctionFactory.class);
    ...
    public static void detectStarRocksFeature(StarRocksSinkOptions sinkOptions) {
        try {
            boolean supportTransactionLoad =
                    isStarRocksSupportTransactionLoad(
                            sinkOptions.getLoadUrlList(),
                            sinkOptions.getConnectTimeout(),
            ...
            LOG.warn(
                    "Can't decide whether StarRocks supports transaction load, and enable it by default.");
    ...
    public static <T> StarRocksDynamicSinkFunctionBase<T> createSinkFunction(
        ...
        switch (sinkVersion) {
            case V1:
                return new FXStarRocksDynamicSinkFunction<>(
                        sinkOptions, schema, rowTransformer, preSql, postSql);
            case V2:
                return new FXStarRocksDynamicSinkFunctionV2<>(
                        sinkOptions, schema, rowTransformer, preSql, postSql);
            default:
                throw new UnsupportedOperationException(
                        "Unsupported sink type " + sinkVersion.name());
        }
    }
    ...
    public static <T> StarRocksDynamicSinkFunctionBase<T> createSinkFunction(
        ...
        switch (sinkVersion) {
            case V1:
                return new FXStarRocksDynamicSinkFunction<>(sinkOptions);
            case V2:
                return new FXStarRocksDynamicSinkFunctionV2<>(sinkOptions);
            default:
                throw new UnsupportedOperationException(
                        "Unsupported sink type " + sinkVersion.name());
        }
    ...
    public static StarRocksSink<RowData> createSink(
        ...
        if (sinkVersion == SinkVersion.V2) {
            ...
            // StarRocksJsonSerializer depends on SinkOptions#supportUpsertDelete which is decided
            // in
            // StarRocksSinkTable#validateTableStructure, so create serializer after validating
            // table structure
            StarRocksISerializer serializer =
                    StarRocksSerializerFactory.createSerializer(
                            sinkOptions, schema.getFieldNames());
        ...
        throw new UnsupportedOperationException(
                "New sink api don't support sink type " + sinkVersion.name());
    }
