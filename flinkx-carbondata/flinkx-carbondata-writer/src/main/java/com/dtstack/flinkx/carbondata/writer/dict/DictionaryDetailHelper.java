...
public class DictionaryDetailHelper {
    ...
    public static DictionaryDetail getDictionaryDetail(
            String dictfolderPath, List<CarbonDimension> primDimensions, String tablePath) {
        ...
        for (int i = 0; i < primDimensions.size(); ++i) {
            ...
            dictFilePaths[i] =
                    CarbonTablePath.getDictionaryFilePath(
                            tablePath, primDimensions.get(i).getColumnId());
            Integer v =
                    fileNamesMap.get(
                            CarbonTablePath.getDictionaryFileName(
                                    primDimensions.get(i).getColumnId()));
            if (v == null) {
